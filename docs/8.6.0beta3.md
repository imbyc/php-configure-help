# 8.6.0beta3 configure help info

```bash
'configure' configures PHP 8.6.0beta3 to adapt to many kinds of systems.

Usage: src/8.6.0beta3/configure [OPTION]... [VAR=VALUE]...
...
SAPI modules:
  ...
  --disable-apache2-conf  Do not activate the PHP module in the Apache
                          httpd.conf during installation via apxs. Useful when
                          installing into a staging directory or when Apache
                          configuration is managed separately (e.g., via
                          a2enmod).
...
Extensions:
  ...
  --with-uhdr             GD: Enable UltraHDR support (only for bundled libgd)
  ...
  --enable-gd-jis-conv    GD: Enable JIS-mapped Japanese font support (only
                          for bundled libgd)
  --with-heif             GD: Enable HEIF support (only for bundled libgd)
  --with-imagequant       GD: Enable libimagequant support (only for bundled
                          libgd)
  --with-tiff             GD: Enable TIFF support (only for bundled libgd)
...
Some influential environment variables:
  ...
  UHDR_WRITE_XMP
              value of UHDR_WRITE_XMP for libuhdr, overriding pkg-config
...
- 版本：8.6.0beta3
- 时间：10 September 2026
- [下载地址](https://downloads.php.net/~svpernova09/php-8.6.0beta3.tar.gz)
- sha256：2499bb217d77566319c0e971a36041484b8cb7181ef5adf282a751e132015e8c
- md5：
